class Star {
  ...
  void display() {
    //Retrieve brightness value from camera
  //cameraLight.loadPixels();
  //for (int y = 0; y < cameraLight.height; y++) {
  //  for (int x = 0; x < cameraLight.width; x++) {
  //    int loc = x + y * cameraLight.width;
  //    color c = cameraLight.pixels[loc];
  //    float opaBright = brightness(c)/255;   
